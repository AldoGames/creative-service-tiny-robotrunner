...
namespace Unity.Tiny
{
    public class EntityGroupSetupVisitor : UTinyProject.Visitor, IDisposable
    {
        ...
        private static bool ActiveInHierarchy(UTinyEntity entity)
        {
            if (!entity.Enabled)
            {
                return false;
            ...
            UTinyObject transform;
            var current = entity;

            while (null != (transform = current.GetComponent(registry.GetTransformType())))
            {
                var parent = transform.GetProperty<UTinyEntity.Reference>("parent").Dereference(registry);
                if (null == parent)
                {
                    break;
                }
        ...
        public static void WriteEntityGroupSetupFunction(UTinyCodeWriter writer, UTinyProject project, UTinyEntityGroup entityGroup, bool writeEntityGroupComponent = true, bool writeEntityLayer = true)
        {
            ...
            using (writer.Scope("function(w)"))
            {
                ...
                foreach (var reference in entityGroup.Entities)
                {
                    ...
                    if (!ActiveInHierarchy(entity))
                    {
                        writer.Line($"e{entityIndex}.setEnabled(false);");
                    }
                }
            ...
            ICustomVisit<Font>
        {
            ...
            void ICustomVisit<int>.CustomVisit(int value)
            {
                ...
                else if (Property.Name == "layer")
                {
                    ...
                    var obj = (m_Container as UTinyObject.PropertiesContainer)?.ParentObject ?? null;
                    if (obj != null && obj.Type.Equals(obj.Registry.GetExtraSortingType()))
                    {
